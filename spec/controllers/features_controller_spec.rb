...
describe FeaturesController do
  ...
  describe "#create" do
  	...
  	context "when it is an ajax request" do
  		it "saves the built feature" do
        pro = mock_model(Project).as_null_object

  			Project.stub(:find).and_return(pro)
  			pro.features.stub(:build).and_return(feature)
  			
        feature.should_receive(:save)
				post :create, :project_id => project, format: :js, :feature => { :name => "new", :description => "new" }
  		end
  	end
